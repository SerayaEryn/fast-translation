...
module.exports = class Translator {
  ...
  _getTranslator(options, key) {
    ...
    const enrichedKey = `${options.namespace || 'default'}${options.language}${key}_plural${number}`;
  ...
  _buildKey(namespace, language, key) {
    if (key.includes('_')) {
      if (key.endsWith('_plural')) {
        ...
        return `${namespace}${language}${key}${number}`;
      } else {
        return `${namespace}${language}${key}`;
      }
    } else {
      const number = this.numbers[language][0];
      return `${namespace}${language}${key}_plural${number}`;
    }
  }
}